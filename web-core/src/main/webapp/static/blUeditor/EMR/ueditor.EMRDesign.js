...
(function (win, UE) {
    ...
    UE.plugins["tl_text"] = function () {
        var me = this, thePlugins = 'tl_text', dialog = getDialog(me, thePlugins, "text.html?temp=" + new Date().getTime(), "文本输入框", 'width:600px;height:350px;', [
            {
    ...
    UE.plugins["tl_select"] = function () {
        var me = this, thePlugins = 'tl_select', dialog = getDialog(me, thePlugins, "select.html?temp=" + new Date().getTime(), "下拉单/多选输入框", 'width:600px;height:350px;', [
            {
                className: 'edui-okbutton',
                label: '确定',
                onclick: function () {
        ...
        if (win.EMR.isdesign) {
            me.addListener('mouseover', function (t, evt) {
                ...
                if (model == '' || model == undefined) {
                    ...
                    /***************QB 开始*****************/
                    var isD=json.txtCheck;
                    var fooldds="多选输入框";
                    if(isD==1||isD==null||isD==""||typeof(isD)=="undefined"){
                    	fooldds="单选输入框";
                    }
                   // $G('txtCheck').ch//QB
                   /***************QB 结束*****************/
                    if (json && json.TYPE == 'select') {
                        var html = popup.formatHtml('<nobr>' + json.NAME + ' '+fooldds+': <span onclick=$$._edit() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
                        if (html) {
                            popup.getDom('content').innerHTML = html;
                            popup.anchorEl = el;
                            popup.showAnchor(popup.anchorEl);
    ...
    UE.plugins["tl_templatecontrol"] = function () {
        var me = this, thePlugins = 'tl_templatecontrol', dialog = getDialog(me, thePlugins, "templatecontrol.html?temp=" + new Date().getTime(), "控件模板选择", 'width:600px;height:350px;', [
            {
                className: 'edui-okbutton',
                label: '确定',
                onclick: function () {
    ...
    UE.registerUI('tl_text', function (editor, uiName) {
        ...
        var dialog = getDialog(editor, uiName, "text.html?temp="+new Date().getTime(), "文本输入框", 'width:600px;height:350px;', [
            {
                className: 'edui-okbutton',
                label: '确定',
                onclick: function () {
    ...
    UE.registerUI('tl_select', function (editor, uiName) {
        var dialog = getDialog(editor, uiName, "select.html?temp=" + new Date().getTime(), "下拉单选输入框", 'width:600px;height:350px;', [
            {
                className: 'edui-okbutton',
                label: '确定',
                onclick: function () {
