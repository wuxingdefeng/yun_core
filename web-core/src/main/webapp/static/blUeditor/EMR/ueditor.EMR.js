    ...
    var text = {
        tlclick: function (t) {
            ...
            try{
            	...
            	jsonstr=jsonstr.replace(/quot;/g,"\"");
            	json=JSON.parse(jsonstr);
            }
...
(function (window, undefined) {
    ...
    function showHTML(t, data, va) {
        ...
        var width = t.offsetWidth, height = t.offsetHeight, left = getLeft(t), top = getTop(t) + height;
        var html = [];
        html.push('<ul class="tl-ui-select-content" style="cursor:pointer;">');
        /*****************QB 开始***************/
        var json=null;
        var model=$(t).parent().attr("tl-model");
        try{
        	json=JSON.parse(model);
        }catch(Err){
        	model=model.replace(/quot;/g,"\"");
        	json=JSON.parse(model);
        }
        var txtChecks=json.txtCheck;
        
        var txtCheckB="EMR.select.tlselectechange(this);";//单选
        if(txtChecks==2){//多选
            txtCheckB="EMR.select.tlselectechange2(this);";
        }
        var Tvalues=$(t).html();
        /*****************QB 结束***************/
        for (var i = 0,l=data.length; i < l; i++) {
            /*************QB 开始***************/
            if(txtChecks==2){//多选
            	var vluss=Tvalues.split(",");
            	var bls=false;
            	if(Tvalues.indexOf(data[i].TEXT)==0||Tvalues.indexOf(data[i].TEXT+",")>0||Tvalues.indexOf(","+data[i].TEXT)>0){
            		bls=true;
            	}
            	if(bls){
            		html.push('<li onclick="'+txtCheckB+'" style="padding:5px 15px;background-color:#08c;color:#fff;"');
                    html.push(' tl-value="' + data[i].VALUE + '" ');
                    html.push(' tl-text="' + data[i].TEXT + '" ');
                    html.push(' title="' + data[i].TEXT + '" ');
            	}else{
            		html.push('<li onclick="'+txtCheckB+'" style="padding:5px 15px;"');
                    html.push(' tl-value="' + data[i].VALUE + '" ');
                    html.push(' tl-text="' + data[i].TEXT + '" ');
                    html.push(' title="' + data[i].TEXT + '" ');
            	}
            }else{//单选
            	if(data[i].TEXT==Tvalues){
            		html.push('<li onclick="'+txtCheckB+'" style="padding:5px 15px;background-color:#08c;color:#fff;"');
                    html.push(' tl-value="' + data[i].VALUE + '" ');
                    html.push(' tl-text="' + data[i].TEXT + '" ');
                    html.push(' title="' + data[i].TEXT + '" ');
...
//            html.push(' class="tl-ui-select-item" ');
        ...
        /*******************QB 开始******************/
        //主容器左边的距离
        var MAINLEFT=null;//QB
//        var TOP=$("#main").offset().top;//QB
       
//        if(TOP!=null&&TOP!=""&&typeof(TOP)!="undefined"){//QB
//	           top=top-TOP;
//        }
        /*******************QB 结束******************/
      
        newNode.setAttribute('class', "tl-ui-select");
        if(typeof($(t).closest("p").html())!="undefined"){
        	MAINLEFT=$("#main").offset().left;
            if(MAINLEFT!=null&&MAINLEFT!=""&&typeof(MAINLEFT)!="undefined"){//QB
  	             left=left-MAINLEFT;
            }
            //newNode.style.top = (top+3) + 'px';//原生
        }else{
        	MAINLEFT=$(t).closest("td").offset().left;
        	 if(MAINLEFT!=null&&MAINLEFT!=""&&typeof(MAINLEFT)!="undefined"){//QB
  	             left=left-MAINLEFT;
            }
        }
        newNode.style.position='absolute';//QB
        newNode.style.backgroundColor='#faf4ff';//QB
        newNode.style.marginLeft= left + 'px';//QB
        newNode.style.border='1px solid #e0e0e0';//QB
        newNode.style.color='#000';//QB
        //newNode.style.padding="5px";//QB
       // newNode.style.paddingLeft="10px";//QB
        //newNode.style.left= left + 'px';//原生
//        newNode.style.marginTop = top + 'px';//QB
        newNode.style.overflow = 'hidden';
//        newNode.style.maxHeight = (height * data.length) + 'px';//QB
//        newNode.style.minWidth = width + 'px';//QB
        newNode.style.display = 'block';
       
        newNode.innerHTML = html.join('');
        //document.body.insertBefore(newNode, document.body.childNodes[0]);
        $(t).parent().after(newNode);//QB
        //t.insertBefore(newNode, t.childNodes[t.childNodes.length-1]);//原代码
...
//            showHistory(t);QB
           ...
           var node= t.parentNode.parentNode.parentNode.parentNode.getAttribute('tl-model');
            ...
            if (json != null) {
                json.VALUE = value;
                json.TEXT = value;
                t.parentNode.parentNode.parentNode.parentNode.setAttribute('tl-model', JSON.stringify(json));//原生
            }
            //t.parentNode.parentNode.parentNode.innerHTML = text;//原生
            $(t).closest("div").prev().find(".tl-value").html(text);//QB
        },tlselectechange2:function(t){//QB 添加的整个函数
            //保存网页
            var value = t.getAttribute('tl-value'), text = t.getAttribute('tl-text');
           var node= t.parentNode.parentNode.parentNode.parentNode.getAttribute('tl-model');
           var json = null;
           try{
            ...
            //t.parentNode.parentNode.parentNode.innerHTML = text;//原生
            var tObj=$(t).closest("div").prev().find(".tl-value");
            if($(tObj).html()==""||$(tObj).html()==null){
            	$(tObj).html(text);
            }else{
            	var tObjHTML=$(tObj).html().trim();
            	if(tObjHTML.indexOf(text+",")==0){
            		$(tObj).html(tObjHTML.replace(text+",",""));
            	}else{
            		if(tObjHTML==text){
                		$(tObj).html("&nbsp;");
                	}else if(tObjHTML.indexOf(text)>0){
                		$(tObj).html(tObjHTML.replace(","+text,""));
                	}else{
                		if(tObjHTML!="&nbsp;"){
                			$(tObj).html(tObjHTML+(","+text));
                		}else{
                			$(tObj).html(text);
                		}
                	}
